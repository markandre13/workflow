...
`;
    ...
    (function (Operation) {
        ...
        Operation[Operation["BRING_FIGURES_TO_FRONT"] = 4] = "BRING_FIGURES_TO_FRONT";
        Operation[Operation["BRING_FIGURES_TO_BACK"] = 5] = "BRING_FIGURES_TO_BACK";
        Operation[Operation["BRING_FIGURES_FORWARD"] = 6] = "BRING_FIGURES_FORWARD";
        Operation[Operation["BRING_FIGURES_BACKWARD"] = 7] = "BRING_FIGURES_BACKWARD";
        Operation[Operation["TRANSFORM_FIGURES"] = 8] = "TRANSFORM_FIGURES";
        Operation[Operation["UPDATE_FIGURES"] = 9] = "UPDATE_FIGURES";
        Operation[Operation["MOVE_HANDLE"] = 10] = "MOVE_HANDLE";
    })(Operation || (Operation = {}));
    class CacheEntry {
    ...
    class FigureEditor extends toad.exports.ModelView {
        ...
        removeFromLayerWithIndex(layer, figures) {
            const result = { figures: [], index: [] };
...
`;
    ...
    class LocalDrawingModel {
        ...
        bringToFront(layerID, figureIds) {
            const fastFigureIds = this.figureIdsAsSet(figureIds); // FIXME: could use the FigureEditor cache instead
            const layer = this.layerById(layerID);
        ...
        bringForward(layerID, figureIds) {
            const fastFigureIds = this.figureIdsAsSet(figureIds); // FIXME: could use the FigureEditor cache instead
            const layer = this.layerById(layerID);
            const removed = this.removeFromLayer(layer, fastFigureIds);
            removed.figures.reverse();
        ...
        bringBackward(layerID, figureIds) {
            const fastFigureIds = this.figureIdsAsSet(figureIds); // FIXME: could use the FigureEditor cache instead
            const layer = this.layerById(layerID);
            const removed = this.removeFromLayer(layer, fastFigureIds);
            removed.figures.reverse();
        ...
        initializeIdCounterHelper(figure) {
        ...
        removeFromLayer(layer, figureIds) {
            const result = { figures: [], index: [] };
    ...
    class BoardModel {
        ...
        bringToFront(layerID, indices) { }
        bringToBack(layerID, indices) { }
        bringForward(layerID, indices) { }
        bringBackward(layerID, indices) { }
    }
